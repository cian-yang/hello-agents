...
最经典的算法是 PPO(Proximal Policy Optimization)，训练目标是:
...
- **奖励函数$R(s,a)$、**
  ...
  $$
- 折扣因子$\gamma$：在**某个状态下执行某个动作后，环境返回的即时奖励，用来衡量该行为的好坏，并指导策略优化**。**定义目标**，强化学习的本质不是学习“正确答案”，而是学习“如何最大化奖励”，因此 **r 的设计决定了智能体最终行为**。
...
3. 奖励函数通常设计为 $r(q, c, a) = 1$ if $a = a^*$ else $0$，训练目标是 $\max_\theta \mathbb{E}_{q, (c,a) \sim \pi_\theta} [r(q, c, a)]$。
   * 奖励函数的意思是： **只有当最终答案正确时，整个推理链才获得奖励 1，否则奖励 0** 。
     * **a** *：参考答案（ground truth）
     * **a** ：最终答案
     * **作用** ：
       * **评价策略好坏** ：奖励函数告诉模型哪个推理链是有价值的。
       * **强化正确行为** ：模型会学习生成能够得到正确答案的推理链。
       * **鼓励探索新的推理路径** ：只要新生成的链能得到正确答案，就能获得奖励。
   * 训练目标：训练模型生成  **正确答案的推理链的概率最大化** 。
     * **πθ** ：模型策略，生成推理链和答案的分布。
     * **E[...]** ：期望值，即平均表现。
     * **目标** ：找到参数 θ，使得模型生成的推理链  **在所有问题上平均获得最大奖励** 。
4. 通过这种方式，**模型学会生成高质量的推理链，而不仅仅是记忆答案**。

![1777937919977](image/1777937919977.png)

#### 工具使用(Tool Use)

**工具使用(Tool Use)**：是指**智能体调用外部工来完成任务的能力**。

在工具使用任务中，行动空间扩展为 $a_t \in \{a_t^{\text{think}}, a_t^{\text{tool}}\}$,

- $a_t^{\text{think}}$ 是生成思考过程,
- $a_t^{\text{tool}} = (\text{tool\_name}， \text{arguments})$ 是调用工具。

**强化学习让智能体学会何时需要使用工具、选择哪个工具、如何组合多个工具**。

例如，在解决数学问题时，智能体需要学会何时使用计算器、何时使用代码解释器、何时直接推理。

#### 记忆(Memory)

记忆(Memory)：指智能体保持和重用过去信息的能力，对于长期任务至关重要。

LLM 的上下文窗口有限，静态检索策略(如 RAG)无法针对任务优化。

强化学习**让智能体学会记忆管理策略：决定哪些信息值得记住、何时更新记忆、何时删除过时信息**。

这类似于人类的工作记忆，我们会主动管理大脑中的信息，保留重要的、遗忘无关的。
...
提示工程使用静态规划模板，难以适应新情况。

强化学习**让智能体学会动态规划：通过试错发现有效的行动序列，学会权衡短期和长期收益**。

例如，在多步任务中，智能体可能需要先执行一些看似"绕路"的步骤，例如收集信息，才能最终完成任务。

#### 自我改进(Self-Improvement)

自我改进(Self-Improvement)：指智能体回顾自身输出、纠正错误并优化策略的能力。

强化学习**让智能体学会自我反思：识别自己的错误、分析失败原因、调整策略**。

这种能力使得智能体能够在没有人工干预的情况下持续改进，类似于人类的"从错误中学习"。

#### 感知(Perception)

**感知(Perception)：指理解多模态信息的能力**。

例如，强化学习可以提升视觉推理能力，让模型学会使用视觉工具，学会视觉规划。

这使得智能体不仅能理解文本，还能理解和操作视觉世界。

# HelloAgents 的 Agentic RL 设计

## 技术选型

技术选型上，

- 我们集成了 TRL(Transformer Reinforcement Learning)框架，
- TRL 是 Hugging Face 的强化学习库，成熟稳定、功能完整、易于集成。
- 模型选择 Qwen3-0.6B：
  - Qwen3-0.6B 是阿里云的小型语言模型，
  - 0.6B 参数适合普通 GPU 训练，性能优秀且开源免费。

## 架构设计

最底层是 **数据集层，负责数据加载和格式转换**，包含

- GSM8KDataset 类、
- create_sft_dataset()函数
- create_rl_dataset函数

第二层是**奖励函数层，负责定义什么是好的行为**，包含

- MathRewardFunction 基类、
- AccuracyReward准确率奖励、
- LengthPenaltyReward 长度惩罚
- StepReward步骤奖励，
- 以及便捷创建函数 create_*_reward()

第三层是 **训练器层，负责具体的训练逻辑和 LoRA 支持**，包含

- SFTTrainerWrapper
- GRPOTrainerWrapper

最顶层是**统一接口层 ，提供 RLTrainingTool 统一训练工具，支持四种操作**:

- action="train"(训练模型)、
- action="load_dataset"(加载数据集)、
- action="create_reward"(创建奖励函数)、
- action="evaluate"(评估模型)。

![1776346435214](image/1776346435214.png)
