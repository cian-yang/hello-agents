...
**推理(Reasoning)**：智能体根据给定问题，生成逻辑步骤（推理链）并得出最终答案的能力。

- 旧方式缺点：

  - 传统的 CoT （思维链）提示方法依**赖少样本示例，泛化能力有限**;
  - **SFT** 只能**模仿训练数据**中的推理模式，**难以创新**。
- RL优势：**通过试错学习有效的推理策略，发现训练数据中没有的推理路径，学会何时需要深度思考、何时可以快速回答**。

  - 将 问题q 分析生成 推理链c 和 答案a，
- 把推理任务用 **MDP / RL 框架** 表示：所以生成推理链  **等价于一条序列决策轨迹** 。

  - **状态（s）** ：到目前为止生成的推理链 `c_1, ..., c_t`。
  - **动作（a）** ：生成下一个推理步骤 `c_{t+1}` 或输出最终答案。
  - **策略（πθ）** ：模型生成下一步的概率分布（参数 θ）。
  - **序列** ：整个推理链 `c = (c_1, c_2, ..., c_n)` + 最终答案 `a`。
- 奖励函数： **只有当最终答案正确时，整个推理链才获得奖励 1，否则奖励 0** 。
- 训练目标：训练模型生成  **正确答案的推理链的概率最大化**
- 流程如图

  ![1777937887734](image/1777937887734.png)

**工具使用(Tool Use)**：是指**智能体调用外部工来完成任务的能力**。RL**让智能体学会何时需要使用工具、选择哪个工具、如何组合多个工具**。

**记忆(Memory)**：指智能体保持和重用过去信息的能力，RL**让智能体学会记忆管理策略：决定哪些信息值得记住、何时更新记忆、何时删除过时信息**。

规划(Planning)：指制定行动序列以达成目标的能力

- 旧方式缺点：传统的 CoT 是线性思考，无法回溯；提示工程使用静态规划模板，难以适应新情况。
- RL**让智能体学会动态规划：通过试错发现有效的行动序列，学会权衡短期和长期收益**。

**自我改进(Self-Improvement)**：指智能体回顾自身输出、纠正错误并优化策略的能力。RL**让智能体学会自我反思：识别自己的错误、分析失败原因、调整策略**。

**感知(Perception)：指理解多模态信息的能力**。强化学习可以提升视觉推理能力，让模型学会使用视觉工具，学会视觉规划。

## Agentic RL 设计

最底层是 **数据集层，负责数据加载和格式转换**

第二层是**奖励函数层，负责定义什么是好的行为**

第三层是 **训练器层，负责具体的训练逻辑和 LoRA 支持**

最顶层是**统一接口层 ，提供 RLTrainingTool 统一训练工具**

![1776346435214](https://file+.vscode-resource.vscode-cdn.net/d%3A/studyDoc/hello-agents/my-notes/image/1776346435214.png)

## 第一层：数据集层

GSM8K 数学推理数据集，**数学推理是评估 LLM 推理能力的理想任务**。

- **数学问题有明确的正确答案**，**可以自动评估**
- **解决数学问题需要分解问题、逐步推导**，这正是**多步推理的典型场景**
- **学到的推理能力可以迁移到其他领域**，具有很强的**泛化性**
- 如果是其他的开放性回答任务，其答案质量难以客观评估，需要大量人工标注。

需要将数据集格式进行转换，适配不同的训练方法

- SFT格式：prompt + completion
- RL格式：prompt + ground_truth

## 第二层：**奖励函数层**

作用：定义了什么是"好的行为"

**准确率奖励**：只关心**答案是否正确**。**数学定义为：这是一个二值奖励函数，答案正确得 1 分，错误得 0 分**。

- 优点：简单直接，容易理解和实现，适合有明确正确答案的任务
- 缺点：**奖励稀疏，只有答案完全正确才有奖励**，**无法区分"接近正确"和"完全错误"，导致训练初期缺乏有效反馈。**

**长度惩罚**：鼓励模型**生成简洁的回答，避免冗长啰嗦**。

- 前提：答案正确
- 优点：**鼓励简洁表达，避免模型生成冗余内容，可以控制推理成本(更短的输出意味着更少的 token 消耗)**。
- 缺点：抑制详细推理

步骤奖励：**鼓励模型生成清晰的推理步骤，提高可解释性**。

- 前提：答案正确
- 优点：**鼓励可解释的推理**，生成的答案更容易验证和调试，**有助于模型学习系统化的思考方式**。
- 缺点：**导致模型为了获得更多奖励生成冗余步骤**，需要平衡步骤数量和答案质量，步骤检测可能不准确。

组合策略：

- **准确率 + 长度惩罚：鼓励简洁正确的答案，适合对话系统、问答系统**。
- **准确率 + 步骤奖励：鼓励详细的推理过程，适合教育场景、可解释 AI**。
...
![1776512919957](https://file+.vscode-resource.vscode-cdn.net/d%3A/studyDoc/hello-agents/my-notes/image/1776512919957.png)

LoRA微调：只训练少量的额外参数，而保持原模型参数冻结。
...
**PPO(Proximal Policy Optimization)** 是最经典的策略优化算法之一，属于 **策略梯度类（Policy Gradient）** 方法。

- 核心目标：在更新策略时尽量保持新策略与旧策略接近，同时提高累积奖励
- 为什么提出这个算法：
  - 传统策略梯度方法（REINFORCE / Vanilla PG）：
    - 更新策略幅度可能过大 → 导致训练不稳定
    - 收敛慢或容易发散
  - PPO 提出了解决方案：
    - **限制策略更新幅度** （Proximal）
    - 使用 **裁剪（clipping）** 或 **KL penalty** 约束策略改变
- **核心思想** ：**别走太远，慢慢来**
  - 如果新策略动作概率变化不大 → 直接使用原梯度
  - 如果变化过大 → 裁剪梯度，避免破坏旧策略的经验

**GRPO** （Goal/Generalized Reward PPO）是  **针对带目标或稀疏奖励问题的 PPO 改进版本** 。

- 目标：让策略在不同目标条件下，也能稳定优化累积奖励，并解决稀疏奖励问题。
- 稀疏奖励是指奖励信号分布不密集、在多数状态下为 0，仅在关键步骤或达成目标时才提供非零反馈的奖励设计。

PPO 是稳定策略梯度优化算法，通过裁剪约束策略更新，GRPO 在此基础上引入目标条件和奖励泛化，使策略能在多目标或稀疏奖励场景下稳定训练。
...
![1777940357930](https://file+.vscode-resource.vscode-cdn.net/d%3A/studyDoc/hello-agents/my-notes/image/1777940357930.png)
...
- 生成参数
  ...
  - `num_generations`: 每个问题生成多少个答案。越多越好，但计算成本也越高。
  - `max_new_tokens`: 每个答案最多生成多少个 token。
  - `temperature`: 生成温度，GRPO 建议 0.7-1.0，保持一定的探索性。
- 优化参数
...
KL 散度在 GRPO 中是衡量当前策略与参考策略差异的指标，通过在损失函数中加入 KL 惩罚，可以**限制策略更新幅度，防止策略过度偏离参考模型，从而保持训练稳定并保留 SFT 学到的知识**。

![1777967703565](https://file+.vscode-resource.vscode-cdn.net/d%3A/studyDoc/hello-agents/my-notes/image/1777967703565.png)
...
评估指标分为三类：准确性指标、效率指标、质量指标。

- 准确性指标：
  - 准确率(Accuracy)
  - Top-K 准确率：**生成 K 个答案，只要有一个正确就算对**。
  - 数值误差(Numerical Error)：**区分"接近正确"(如预测 72.5，真实 72)和"完全错误"(如预测 100，真实 72)**。**计算预测值与真实值的误差**。
- 效率指标：
...
![1776518021889](https://file+.vscode-resource.vscode-cdn.net/d%3A/studyDoc/hello-agents/my-notes/image/1776518021889.png)
...
![1776519124847](https://file+.vscode-resource.vscode-cdn.net/d%3A/studyDoc/hello-agents/my-notes/image/1776519124847.png)
...
- **工具调用能力**评估：智能体需要理解用户意图，选择合适的工具，并正确构造函数调用
...
- **通用能力**评估：**评估智能体在真实世界任务中的综合表现**，包括多步推理、知识运用、多模态理解等能力
...
![1776579812198](https://file+.vscode-resource.vscode-cdn.net/d%3A/studyDoc/hello-agents/my-notes/image/1776579812198.png)
...
![1776580306238](https://file+.vscode-resource.vscode-cdn.net/d%3A/studyDoc/hello-agents/my-notes/image/1776580306238.png)
...
准确率 (Accuracy)：定义为 AST 匹配成功的样本比例

AST 匹配率 (AST Match Rate)：与准确率相同，强调使用 AST 匹配算法

分类准确率 (Category-wise Accuracy)：对于每个类别 $c \in \{\text{simple}, \text{multiple}, \text{parallel}, \ldots\}$，计算该类别的准确率

加权准确率 (Weighted Accuracy)：考虑不同类别的难度权重：

错误率 (Error Rate)：未能正确调用函数的样本比例

BFCL 官方评估工具，BFCL 提供官方 CLI 工具进行评估：
...
采用**三种互补的评估方法：LLM Judge、Win Rate 和人工打分**。

![1776591133422](https://file+.vscode-resource.vscode-cdn.net/d%3A/studyDoc/hello-agents/my-notes/image/1776591133422.png)

### 题目生成

使用 HuggingFace 上的 `TianHongZXY/aime-1983-2025`数据集作为参考，该数据集包含从 1983 年到 2025 年的 900 多道 AIME 真题，为我们的生成和评估提供了丰富的参考样本。

### LLM Judge 评估

**LLM Judge 评估**：**LLM Judge 通过使用大语言模型作为评委（传统为人工）**，LLM Judge 可以提供详细的评分理由和改进建议，帮助我们理解生成数据的优缺点，为后续优化提供方向。
...
Win Rate 评估：衡量生成题目与真题的质量差距

- W**in Rate 评估通过成对对比的方式**，让 LLM 直接判断生成题目和真题哪个更好
...
**对于数学题目这种需要严格逻辑推理的内容，人工验证仍然是不可或缺的**。
...
GAIA 数据集的样本示例可以参考下面的代码片段：关键字段说明：
...
准精确匹配（Quasi Exact Match）评估算法，这是 GAIA 官方定义的评估标准。

核心思想是：**先对答案进行归一化处理，然后进行精确匹配**
...
精确匹配率 (Exact Match Rate)：定义为准精确匹配成功的样本比例：

分级准确率 (Level-wise Accuracy)：对于每个难度级别 $\ell \in \{1, 2, 3\}$，计算该级别的准确率
